...
const isFormValid = computed((): boolean => {
    ...
    if (!form.name || !form.wa_session_id) {
        return false;
    }
    ...
    switch (form.message_type) {
        case 'text':
            return !!form.message_template && form.message_template.trim().length > 0;
        ...
        case 'link':
            return !!form.link_url && form.link_url.trim().length > 0;

        case 'location':
            return form.latitude !== null && form.longitude !== null;

        case 'contact':
            return !!form.contact_name && form.contact_name.trim().length > 0 &&
                !!form.contact_phone && form.contact_phone.trim().length > 0;

        case 'poll':
            return !!form.poll_question && form.poll_question.trim().length > 0 &&
                form.poll_options.length >= 2 &&
                form.poll_options.every(opt => opt && opt.trim().length > 0);
